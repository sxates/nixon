...
Injects CSV-based transcript data into the Nixon SQLite database,
...
def get_default_db_path() -> Path:
    ...
    # The app's data dir is keyed by the BUNDLE IDENTIFIER (ai.vinyl.app), not the product
    # name — the identifier is deliberately unchanged by the 0057 Nixon rename (ADR-0004).
    identifier = "ai.vinyl.app"

    if system == "Darwin":  # macOS
        base_path = Path.home() / "Library" / "Application Support" / identifier
    elif system == "Windows":
        appdata = os.environ.get("APPDATA", "")
        if appdata:
            base_path = Path(appdata) / identifier
        else:
            base_path = Path.home() / "AppData" / "Roaming" / identifier
    else:  # Linux and others
        base_path = Path.home() / ".config" / identifier
...
def main():
    parser = argparse.ArgumentParser(
        description="Inject CSV transcript data into Nixon database",
...
Example usage:
  ...
  python inject_transcript.py --csv data.csv --db ~/custom/path.sqlite
    ...
    if not db_path.exists():
        ...
        print("Make sure Nixon has been run at least once to create the database.", file=sys.stderr)
        sys.exit(1)
    ...
    print("\nThe meeting should now appear in the Nixon sidebar.")
