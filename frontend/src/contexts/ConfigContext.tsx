...
export function ConfigProvider({ children }: { children: ReactNode }) {
  ...
  // Provider-specific API key hints (masked, loaded once at startup)
      ...
      try {
        ...
        if (config) {
          ...
          setTranscriptModelConfig({
            ...
            // Masked hint only — the backend no longer returns raw keys (0030 WS2)
            ...
            try {
              ...
              if (customConfig) {
                ...
                setModelConfig(prev => ({
                  ...
                  // Masked hint only (0030 WS2); the real key stays backend-side
                  customOpenAIApiKey: customConfig.apiKeyMasked ?? null,
  ...
  // Load all provider API key statuses on mount. The backend only returns
  // configured/masked info (spec 0030 WS2), so the values here are masked
  // display hints ("••••1234") or null — never raw keys.
  useEffect(() => {
    const loadAllApiKeys = async () => {
      try {
        ...
        const keys = await Promise.all(
          providers.map(p =>
            invoke<ApiKeyStatus>('api_get_api_key', { provider: p })
              .then(status => apiKeyHint(status))
        ...
        console.log('[ConfigContext] Loaded provider API key statuses');
      } catch (error) {
        console.error('[ConfigContext] Failed to load provider API key statuses:', error);
      }
    };
  ...
  useEffect(() => {
    return safeListen<ModelConfig>('model-config-updated', (event) => {
      ...
      // Update provider-specific key hint when config changes. Mask before
      // storing so a freshly typed key never sits in long-lived context state.
      if (event.payload.apiKey && event.payload.provider !== 'custom-openai') {
        updateProviderApiKey(event.payload.provider, maskApiKey(event.payload.apiKey));
      }
    });
  }, []);
