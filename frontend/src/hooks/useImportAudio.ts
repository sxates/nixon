...
export function useImportAudio({
  ...
  useEffect(() => {
    ...
    const setupListeners = async () => {
      // Progress events
      const unlistenProgress = makeSafeUnlisten(await listen<ImportProgress>(
        'import-progress',
        (event) => {
      ...
      // Completion event
      const unlistenComplete = makeSafeUnlisten(await listen<ImportResult>(
        'import-complete',
        async (event) => {
          if (isCancelledRef.current) return;
      ...
      // Error event
      const unlistenError = makeSafeUnlisten(await listen<ImportError>(
        'import-error',
        async (event) => {
          if (isCancelledRef.current) return;
