...
export interface ModelConfig {
  ...
  whisperModel: string;
  /**
   * Write-only key field (spec 0030 WS2): holds either a masked display hint
   * ("••••1234") for an already-stored key, or a replacement the user just
   * typed. The backend never returns raw keys and ignores masked echoes.
   */
  apiKey?: string | null;
  /** Whether a key is stored for `provider` (from the backend getter). */
  apiKeyConfigured?: boolean;
  /** Masked display hint for the stored key, e.g. "••••1234". */
...
export interface CustomOpenAIConfig {
  endpoint: string;
  /** Write-only: sent on save; the getter returns apiKeyConfigured/apiKeyMasked instead. */
  apiKey: string | null;
  model: string;
  maxTokens: number | null;
  temperature: number | null;
...
/** Shape returned by `api_get_custom_openai_config` (key masked, spec 0030 WS2). */
export interface CustomOpenAIConfigStatus {
  endpoint: string;
  model: string;
  maxTokens: number | null;
  temperature: number | null;
...
export class ConfigService {
  ...
  /**
   * Get custom OpenAI configuration (API key masked, spec 0030 WS2)
   * @returns Promise with CustomOpenAIConfigStatus or null if not configured
   */
