...
class IndexedDBService {
  ...
  private readonly DB_NAME = 'NixonRecoveryDB';
  /** Pre-0057 database name, deleted on init so old payloads don't linger. */
  private readonly LEGACY_DB_NAME = 'MeetilyRecoveryDB';
  private readonly DB_VERSION = 1;
  private initPromise: Promise<void> | null = null;
  private legacyDeleteAttempted = false;

  /**
   * Best-effort removal of the pre-0057 database (`MeetilyRecoveryDB`). Fire and
   * forget: any failure — including environments without `indexedDB`, such as
   * tests — is ignored, since this is cleanup and never load-bearing.
   */
  private deleteLegacyDatabase(): void {
    if (this.legacyDeleteAttempted) return;
    this.legacyDeleteAttempted = true;
    try {
      const request = indexedDB.deleteDatabase(this.LEGACY_DB_NAME);
      // Swallow the async outcomes too; a blocked delete just retries next launch.
      request.onerror = () => {};
      request.onblocked = () => {};
    } catch {
      // No-op: `indexedDB` unavailable or delete rejected outright.
    }
  }
  ...
  async init(): Promise<void> {
    ...
    this.initPromise = new Promise((resolve, reject) => {
      try {
        // `MeetilyRecoveryDB` is the pre-0057 name of this database; drop any
        // leftover copy once, best-effort, so its payload can't leak forever.
