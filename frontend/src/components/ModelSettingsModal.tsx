...
import { cn, isMaskedApiKey, isOllamaNotInstalledError, maskApiKey } from '@/lib/utils';
...
export interface ModelConfig {
   ...
   * stored key, or a replacement the user just typed. The backend never
   * returns raw keys and ignores masked echoes on save.
   */
  apiKey?: string | null;
...
export function ModelSettingsModal({
  ...
  const [_openRouterError, setOpenRouterError] = useState<string>('');
  ...
  const [_autoGenerateEnabled, setAutoGenerateEnabled] = useState<boolean>(true); // Default to true
    ...
    const fetchModelConfig = async () => {
      ...
      try {
        ...
        if (data && data.provider !== null) {
          // The backend returns configured/masked key status, never the raw
          // key (spec 0030 WS2). Show the masked hint; typing replaces it.
          if (data.provider !== 'ollama') {
            data.apiKey = data.apiKeyConfigured ? (data.apiKeyMasked ?? '••••') : null;
            setApiKey(data.apiKey);
            setIsApiKeyLocked(!!data.apiKey);
          }
          setModelConfig(data);
          ...
          if (data.provider === 'custom-openai') {
            try {
              ...
              if (customConfig) {
                ...
                // Masked hint only (0030 WS2); backend keeps the stored key
                // when a masked/empty value is saved back.
                setCustomOpenAIApiKey(customConfig.apiKeyMasked || '');
  ...
  const handleSave = async () => {
    ...
    // Update provider-specific key hint in context (masked — raw keys are
    // write-only and never kept in long-lived state, spec 0030 WS2)
    if (updateProviderApiKey && updatedConfig.apiKey && updatedConfig.provider !== 'custom-openai') {
      updateProviderApiKey(updatedConfig.provider, maskApiKey(updatedConfig.apiKey));
    }
  ...
  return (
    <div>
                ...
                if (provider === 'custom-openai') {
                  invoke<any>('api_get_custom_openai_config').then((config) => {
                    if (config) {
                      ...
                      setCustomOpenAIApiKey(config.apiKeyMasked || '');
              ...
              <div className="absolute inset-y-0 right-0 pr-1 flex items-center space-x-1">
                {apiKey?.trim() && (
                  <Button
                    ...
                    size="icon"
                    onClick={() => {
                      // Unlocking a masked (stored) key clears the field so the
                      // user types a full replacement — the raw stored key is
                      // never available to edit in place (spec 0030 WS2).
                      if (isApiKeyLocked && isMaskedApiKey(apiKey)) {
                        setApiKey('');
                      }
                      setIsApiKeyLocked(!isApiKeyLocked);
                    }}
                    className={isLockButtonVibrating ? 'text-destructive' : ''}
                    title={
                      isApiKeyLocked
            ...
            {!isEndpointSectionCollapsed && (
              <>
                ...
                {ollamaEndpointChanged && !error && (
                  <Alert className="mt-3 border-brand/50 bg-brand/10">
                        ...
                        {isDownloading('gemma3:1b') && getProgress('gemma3:1b') !== undefined && (
                          <div className="bg-card rounded-md border p-3">
                            <div className="flex items-center justify-between mb-2">
                              <span className="text-sm font-medium text-brand">Downloading gemma3:1b</span>
                        ...
                        <div
                          ...
                          className={cn(
                            ...
                            modelConfig.model === model.name
                              ? 'ring-1 ring-brand border-brand bg-brand/10'
                              : 'hover:bg-muted/50',
                        ...
                        >
                          ...
                          {modelIsDownloading && progress !== undefined && (
                            <div className="mt-3 pt-3 border-t border-border">
                              <div className="flex items-center justify-between mb-2">
                                <span className="text-sm font-medium text-brand">Downloading...</span>
      ...
      <div className="mt-6 flex justify-end">
        <Button
          className={cn(
            'px-4 text-sm font-medium text-brand-foreground rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-ring',
            isDoneDisabled ? 'bg-muted text-muted-foreground cursor-not-allowed' : 'bg-brand hover:bg-brand/90'
          )}
          onClick={handleSave}
