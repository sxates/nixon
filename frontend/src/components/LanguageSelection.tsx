...
/**
 * Single global transcription-language control (spec 0038 WS7.d).
 *
 * Language is ONE global backend preference (`set_language_preference`), written
 * here through `useConfig().setSelectedLanguage` (which persists to localStorage
 * and syncs to Rust). This is the only place the user picks a language — the old
 * in-recording "Language Settings" modal was removed. There is deliberately no
 * per-transcript / per-summary language (out of scope).
 */
export function LanguageSelection() {
  ...
  const handleLanguageChange = async (languageCode: string) => {
    setSaving(true);
    try {
      // Save language preference to localStorage and sync to backend (the single
      // global `set_language_preference` — see ConfigContext.handleSetSelectedLanguage).
    ...
    <div className="space-y-4">
      <div className="space-y-2">
        <select
          ...
          disabled={saving}
          className="w-full px-3 py-2 text-sm bg-card border border-input rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-ring focus:border-ring disabled:bg-muted disabled:text-muted-foreground"
        >
