...
// specs/0057 Task 2 — the Day/Week toggle moved from a hand-rolled pill pair to
// the shared SegmentedControl. `aria-pressed` (and the "Agenda view" group label)
// is the contract assistive tech reads, so it is what this pins down.
...
describe('TodayToolbar view toggle', () => {
  it('renders Day/Week as an "Agenda view" group with aria-pressed on the active mode', () => {
    renderToolbar('day');
    const group = screen.getByRole('group', { name: 'Agenda view' });
    expect(within(group).getByRole('button', { name: 'Day' }).getAttribute('aria-pressed')).toBe('true');
  ...
  it('moves aria-pressed when the week view is active', () => {
    ...
    expect(within(group).getByRole('button', { name: 'Day' }).getAttribute('aria-pressed')).toBe('false');
  });
