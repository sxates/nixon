...
impl WhisperEngine {
    ...
    pub fn new_with_models_dir(models_dir: Option<PathBuf>) -> Result<Self> {
        ...
        let models_dir = if let Some(dir) = models_dir {
            ...
            // Development: Use frontend/models directories
            // Production: Use system directories (should be overridden by caller)
            if cfg!(debug_assertions) {
                // Development mode - try frontend model directories
                ...
                crate::app_paths::app_data_dir().join("models")
            }
        };
    ...
    fn calculate_repetition_ratio(text: &str) -> f32 {
        ...
        for word in &words {
            *word_counts.entry(word.to_lowercase()).or_insert(0_usize) += 1;
        }
    ...
    pub async fn transcribe_audio(
        ...
        // "auto"/None => auto-detect; "auto-translate" => translate to English; else use the language code
        let (language_code, should_translate) = match language.as_deref() {
            Some("auto") | None => (None, false),
            Some("auto-translate") => (None, true),
            Some(lang) => (Some(lang), false),
        ...
        if cleaned_result.is_empty() {
            ...
            if cleaned_result != final_result {
                // PRIVACY (specs/0028): never log transcript CONTENT at info! — release logs
                // persist to ~/Library/Logs. Log only lengths; full text is dev-only.
                log::info!(
            ...
            {
                // PRIVACY (specs/0028): length only at info!; content dev-only via perf_debug!.
                log::info!(
