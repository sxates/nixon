...
use super::repositories::meeting_note::MeetingNotesRepository;
...
pub async fn import_and_initialize_database(
    ...
    // The imported DB may carry cleartext API keys — move them to the Keychain
    // (spec 0030 WS1 / ADR-0009, best-effort).
...
/// Saves the user's raw notes for a meeting (autosaved from the live notepad during recording).
/// Does not touch AI-enhanced notes. See spec 0003 (note-enhancement pipeline).
#[tauri::command]
pub async fn api_save_meeting_notes(
    ...
    match MeetingNotesRepository::upsert_notes(
        pool,
        &meeting_id,
    ...
    .await
    {
        ...
        Err(e) => {
            error!("Failed to save meeting notes for {}: {}", meeting_id, e);
            Err(e.to_string())
        }
    }
}

/// Loads a meeting's notes (raw + enhanced), or null if none have been saved yet.
#[tauri::command]
pub async fn api_get_meeting_notes(
    ...
    MeetingNotesRepository::get_notes(pool, &meeting_id)
        .await
        .map_err(|e| {
            error!("Failed to load meeting notes for {}: {}", meeting_id, e);
            e.to_string()
        })
}
