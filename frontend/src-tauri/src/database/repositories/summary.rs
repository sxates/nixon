...
impl SummaryProcessesRepository {
    ...
    /// `speaker_attributed` / `generated_markdown_hash` (specs/0041 WS2): whether this
    /// generation ran over a speaker-attributed transcript, and the fingerprint of the
    /// stored `$.markdown` — inputs to the one-shot post-diarization auto-regenerate
    /// trigger (summary/refresh.rs) and its pristine (not-user-edited) guard.
    /// `speaker_names_hash` (specs/0044 WS3): fingerprint of the resolved speaker
    /// display-name set this generation saw — input to the post-naming refresh trigger.
    #[allow(clippy::too_many_arguments)] // cohesive completion-state param set
    pub async fn update_process_completed(
        ...
        generated_markdown_hash: &str,
        speaker_names_hash: &str,
    ) -> Result<(), sqlx::Error> {
        let now = Utc::now();
        ...
        sqlx::query(
            ...
            SET status = 'completed', result = ?, updated_at = ?, end_time = ?, chunk_count = ?, processing_time = ?, error = NULL, result_backup = NULL, result_backup_timestamp = NULL,
                speaker_attributed = ?, generated_markdown_hash = ?, speaker_names_hash = ?
            WHERE meeting_id = ?
            "#
        )
        .bind(result_str)
        ...
        .bind(speaker_attributed)
        .bind(generated_markdown_hash)
        .bind(speaker_names_hash)
        .bind(meeting_id)
        .execute(pool)
        .await?;
        log_info!(
