...
pub struct SidecarManager {
    ...
    /// specs/0056 W7: one request at a time on the single stdin/stdout pipe. The helper
    /// protocol has no request ids, so without this two concurrent callers (e.g. Ask AI and a
    /// background extraction) each wrote a line and then raced for `stdout_reader` — whoever
    /// won consumed the OTHER caller's reply. Held across write+read; acquired OUTSIDE the
    /// generation timeout so a queued caller is not billed for the one ahead of it.
...
impl SidecarManager {
    ...
    fn resolve_helper_binary() -> Result<PathBuf> {
        ...
        if let Ok(env_path) = std::env::var("NIXON_LLAMA_HELPER") {
            if !env_path.is_empty() {
                let path = PathBuf::from(env_path);
                if path.exists() {
                    log::info!(
                        "Using llama-helper from NIXON_LLAMA_HELPER: {}",
                        path.display()
                    );
                    return Ok(path);
                }
        ...
        Err(anyhow!(
            "llama-helper binary not found. Build with 'cd llama-helper && cargo build --release' or set NIXON_LLAMA_HELPER env var."
        ))
    }
    ...
    pub async fn send_request(&self, request_json: String, timeout: Duration) -> Result<String> {
        ...
        // specs/0056 W7: serialize whole request/response exchanges (see `request_lock`).
    ...
    async fn send_ping(&self) -> Result<()> {
        ...
        // specs/0056 W7: but we DO take the exchange lock — the "no ping while a request is
        // active" check above is check-then-act, and a pong landing between another caller's
        // write and read would be consumed as that caller's reply.
        let _exchange = self.request_lock.lock().await;
