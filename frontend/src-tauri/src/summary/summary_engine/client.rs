...
/// * `options` - Per-call-site overrides layered onto the model's sampling profile
///   (specs/0053). `GenerationOptions::default()` reproduces prior behaviour exactly.
///
/// # Returns
/// Generated text
...
mod tests {
    ...
    #[test]
    fn test_request_serialization_includes_grammar_when_set() {
        let request = Request::Generate {
            prompt: "test prompt".to_string(),
            max_tokens: Some(512),
            context_size: Some(2048),
        ...
        let json = serde_json::to_string(&request).unwrap();
        assert!(json.contains("\"grammar\":\"root ::= \\\"[]\\\"\""));
    }
