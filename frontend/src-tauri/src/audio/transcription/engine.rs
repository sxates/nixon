...
pub async fn validate_transcription_model_ready<R: Runtime>(
    ...
    let config =
        match crate::settings::api_get_transcript_config(app.clone(), app.clone().state()).await {
            Ok(Some(config)) => {
                info!(
            ...
            Ok(None) => {
                ...
                crate::settings::TranscriptConfig {
...
pub async fn get_or_init_whisper<R: Runtime>(
    ...
    if let Some(engine) = existing_engine {
        ...
        if engine.is_model_loaded().await {
            ...
            let configured_model =
                match crate::settings::api_get_transcript_config(app.clone(), app.clone().state())
                    .await
                {
                    Ok(Some(config)) => {
                        info!(
    ...
    let model_to_load = match crate::settings::api_get_transcript_config(
        app.clone(),
        app.clone().state(),
    )
    .await
