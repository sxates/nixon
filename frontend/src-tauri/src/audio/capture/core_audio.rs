...
/// Throttle interval for ring-buffer-overflow warnings emitted from the RT
/// callback (warn once per this many consecutive overflow events).
#[cfg(target_os = "macos")]
const OVERFLOW_WARN_INTERVAL: u32 = 100;
...
struct AudioContext {
    ...
    waker: Arc<AtomicWaker>,
    current_sample_rate: Arc<AtomicU32>,
    /// Count of ring-buffer overflow events; used only to throttle the warning
    /// log. Overflows drop samples but never terminate the tap.
    overflow_count: Arc<AtomicU32>,
    /// Set only on genuine stream teardown (Drop) — reserved for real
    /// device/permission loss so `poll_next` can return `Ready(None)`.
    ...
    pub fn new() -> Result<Self> {
        ...
        let agg_desc = cf::DictionaryOf::with_keys_values(
            ...
            &[
                ...
                cf::str!(c"nixon-audio-tap").as_type_ref(),
                &output_uid,
...
fn process_audio_data(ctx: &mut AudioContext, data: &[f32]) {
    ...
    if pushed < buffer_size {
        // Ring-buffer overflow: the consumer (transcription/mix pipeline) is
        // momentarily falling behind. Drop the samples that didn't fit and KEEP
        // RUNNING — a transient stall must never permanently kill system-audio
        // capture. `Poll::Ready(None)` is reserved for real device/permission
        // loss (signalled via `should_terminate` on Drop). Throttle the warning
        // so we do no heavy work on the real-time callback thread.
        let dropped = buffer_size - pushed;
        let n = ctx.overflow_count.fetch_add(1, Ordering::AcqRel);
        if n.is_multiple_of(OVERFLOW_WARN_INTERVAL) {
            warn!(
                "⚠️ CoreAudio: ring-buffer overflow, dropped {} samples (consumer falling behind); continuing capture",
                dropped
            );
        }
    }
    ...
    if pushed > 0 {
        // Lock-free wake: safe to call from the RT IO-proc thread. `AtomicWaker`
        // takes the registered waker (if any) and wakes it; no lock, no unwrap.
        ctx.waker.wake();
    }
}
...
impl Stream for CoreAudioStream {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        // No data available: register our waker (lock-free) BEFORE re-checking
        // the buffer, so a push racing with this poll can't be lost.
        ...
        // Only end the stream on genuine teardown / device loss — never on
        // transient buffer pressure (overflows drop samples and keep running).
        if self._ctx.should_terminate.load(Ordering::Acquire) {
            warn!("CoreAudioStream terminating (device/stream closed)");
