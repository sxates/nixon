...
mod tests {
    ...
    fn test_calculate_buffer_timeout_bluetooth() {
        ...
        // ~160ms. Assert with a small tolerance rather than exact equality: the
        // computation goes through Duration::mul_f32, whose f32 precision yields
        // 159.999996ms (off by ~4µs) — functionally 160ms but not bit-exact, which
        // made the old assert_eq! flake. Mirror the range-style assert used by the
        // wired case below.
