...
use super::recording_preferences::recordings_root;
...
///
/// `pub(crate)` so the recording saver can probe a merged crash-recovery segment's real
/// duration (specs/0037) without duplicating the symphonia probe.
...
async fn run_import<R: Runtime>(
    ...
    let base_folder = recordings_root();
    ...
    // Persist the full 16 kHz mono buffer as the meeting's `system.wav` so it can be
    // diarized later. Diarization requires a per-channel `system.wav` (the live
    // recorder writes one; import previously did not, so "Identify speakers" failed
    // with "no recording folder recorded"). This buffer is *already* 16 kHz mono —
    // exactly the diarization input format — so it is written verbatim (NOT through
    // `ChannelWavWriter`, which would 48→16 kHz downsample and corrupt it). It also
    // covers the FULL timeline (not just VAD speech chunks), which diarization needs
    // to align transcript timestamps. Best-effort: a failure here must not fail the
    // import (diarization just stays unavailable), like the transcripts/metadata writes.
    let system_wav_path = super::channel_writer::system_channel_wav(&meeting_folder);
    if let Err(e) = super::channel_writer::write_16k_mono_wav(&system_wav_path, &audio_samples) {
        warn!(
            "Failed to write system.wav for diarization ({}): {}",
            system_wav_path.display(),
            e
        );
    }
    ...
    let whisper_engine = if !use_parakeet && total_segments > 0 {
        Some(super::import_engines::get_or_init_whisper(&app, model.as_deref()).await?)
    } else {
        None
    };
    let parakeet_engine = if use_parakeet && total_segments > 0 {
        Some(super::import_engines::get_or_init_parakeet(&app, model.as_deref()).await?)
    } else {
        None
    };
...
async fn create_meeting_with_transcripts(
    ...
    for segment in segments {
        ...
        // Imported audio is a single mixed track with no separate mic channel: tag
        // every segment "system" so diarization treats all speakers as remote and
        // never spuriously assigns "You"/local (see diarization::pipeline channel_from_db).
        .bind("system")
        .execute(&mut *tx)
        .await
...
mod tests {
    ...
    /// Writes a minimal valid 16-bit mono PCM WAV fixture (silence) and returns its path.
    /// Generated at test time so no checked-in sample files are needed.
    fn write_wav_fixture(file_name: &str, duration_seconds: f64) -> std::path::PathBuf {
        const SAMPLE_RATE: u32 = 16_000;
        let n_samples = (duration_seconds * SAMPLE_RATE as f64) as u32;
    ...
    fn test_extract_duration_from_metadata_wav() {
        let path = write_wav_fixture("nixon_import_duration_fixture.wav", 2.0);

        let result = extract_duration_from_metadata(&path);
        let _ = std::fs::remove_file(&path);

        let duration = result.expect("metadata extraction should succeed for a PCM WAV");
        assert!(
    ...
    fn test_validate_audio_file_with_metadata() {
        let path = write_wav_fixture("nixon_import_validate_fixture.wav", 1.0);

        let result = validate_audio_file(&path);
        let _ = std::fs::remove_file(&path);

        let info = result.expect("validation should succeed for a PCM WAV");
    ...
    fn test_split_segment_at_silence_splits_long_segment() {
        ...
        for sample in &mut samples[(25 * 16000)..(25 * 16000 + 3200)] {
            *sample = 0.0;
        }
        let segment = crate::audio::vad::SpeechSegment {
            samples,
