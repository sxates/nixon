...
impl<R: Runtime> ConsentManager<R> {
    ...
    /// (`<app-data-dir>/notifications.json`), rooted at the identifier-derived
    /// data dir so dev/prod stay isolated (ADR-0004).
    fn get_settings_path() -> Result<PathBuf> {
        let path = crate::app_paths::app_data_dir().join("notifications.json");
