...
impl<R: Runtime> SystemNotificationHandler<R> {
    ...
    pub async fn show_notification(&self, notification: Notification) -> Result<()> {
        ...
        // No .icon() on purpose: macOS ignores per-notification icons.
        // tauri-plugin-notification delegates to notify-rust, whose docs say
        // "macOS does not support manually setting the icon" — the banner
        // always wears the icon Notification Center has cached for our bundle
        // id. If that cache holds stale (Meetily) artwork, run
        // scripts/reset-notification-icon-cache.sh (see ADR-0004 gotchas).
        let builder = self
            .app_handle
            .notification()
            .builder()
