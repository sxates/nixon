...
/// Shared process-wide `reqwest::Client` for the LLM provider modules (specs/0028).
///
/// Constructing a `reqwest::Client` per request leaks connection pools and TLS setup
/// cost on every call. A `Client` is internally reference-counted (cheap to clone) and
/// meant to be reused, so all default-configured provider calls share this one. Modules
/// that need bespoke settings (custom timeouts, etc.) still build their own client.
