...
pub fn get_openrouter_models() -> Result<Vec<OpenRouterModel>, String> {
    // NOTE: openrouter uses the *blocking* reqwest client (sync command), so it cannot
    // share the async `config::shared_http_client()`. Kept as a per-call blocking client.
